...
class GameBase(BaseModel):
    first_player_profile_id: uuid.UUID = Field(..., exclude=True)
    second_player_profile_id: uuid.UUID = Field(..., exclude=True)
...
class GameCreate(GameBase):
    @computed_field
    @property
    def winner_player_profile_id(self) -> uuid.UUID:
        if self.first_player_score > self.second_player_score:
            return self.first_player_profile_id
        return self.second_player_profile_id

    @computed_field
    @property
    def looser_player_profile_id(self) -> uuid.UUID:
        if self.first_player_score > self.second_player_score:
            return self.second_player_profile_id
        return self.first_player_profile_id
